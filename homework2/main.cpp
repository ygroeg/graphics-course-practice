...
try
{
  ...
  while (running)
  {
    ...
    // двигаться куда смотрит камера?
    //    x = cos(glm::radians(camera_yaw)) * cos(glm::radians(camera_pitch));
    //    y = sin(glm::radians(camera_pitch));
    //    z = sin(glm::radians(camera_yaw)) * cos(glm::radians(camera_pitch));
