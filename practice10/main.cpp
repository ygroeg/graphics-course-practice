...
void main()
{
    ...
    float cosine = dot(real_normal, to_camera_dir);
    vec3 reflection_dir = to_camera_dir - 2.0 * real_normal * cosine; // знак?
    float x = atan(reflection_dir.z, reflection_dir.x) / PI * 0.5 + 0.5;
    float y = -atan(reflection_dir.y, length(reflection_dir.xz)) / PI + 0.5;
    vec3 envmap_albedo = texture(envmap_texture, vec2(x, y)).rgb;
    ...
    out_color = vec4((lightness * albedo + envmap_albedo) / 2, 1.0);
}
)";
...
void main() {
    ...
    vec3 albedo = texture(bg_texture, vec2(x,y)).rgb;
...
try
{
  ...
  GLuint envmap_texture_location =
      glGetUniformLocation(program, "envmap_texture");

  auto background_vertex_shader =
      create_shader(GL_VERTEX_SHADER, background_vertex_shader_source);
  auto background_fragment_shader =
      create_shader(GL_FRAGMENT_SHADER, background_fragment_shader_source);
  auto background_program =
      create_program(background_vertex_shader, background_fragment_shader);

  GLuint bg_view_projection_inverse_location =
      glGetUniformLocation(background_program, "view_projection_inverse");
  GLuint bg_texture_location =
      glGetUniformLocation(background_program, "bg_texture");
  GLuint bg_camera_position_location =
      glGetUniformLocation(background_program, "camera_position");
  GLuint bg_vao;
  ...
  while (running)
  {
    ...
    glUniformMatrix4fv(bg_view_projection_inverse_location, 1, GL_FALSE,
                       reinterpret_cast<float *>(&view_projection_inverse));
    glUniform1i(bg_texture_location, 2);
    glUniform3fv(bg_camera_position_location, 1,
                 reinterpret_cast<float *>(&camera_position));

    glUniform1i(envmap_texture_location, 2);
