...
vec4 blur(vec2 texcoord)
{
    ...
    return vec4(sum / sum_w);
}

void main()
{
    vec3 pixel = vec3(texture(render_result, vec2(texcoord.x, texcoord.y)));
    switch (mode)
    {
        case 0:
        ...
        case 1:
            ...
            break;
        case 2:
            pixel = vec3(texture(render_result, texcoord + vec2(sin(texcoord.y * 50.0
                        + time) * 0.01, 0.0)));
            out_color = vec4(pixel, 1.0);
            break;
        case 3:
            out_color = blur(texcoord);
            break;
...
try
{
  ...
  auto dragon_vertex_shader =
      create_shader(GL_VERTEX_SHADER, dragon_vertex_shader_source);
  auto dragon_fragment_shader =
      create_shader(GL_FRAGMENT_SHADER, dragon_fragment_shader_source);
  auto dragon_program =
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width / 2, height / 2, 0,
               GL_RGBA, // если GL_RGBA8, то Framebuffer incomplete
  ...
  while (running)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      case 0:
      {
        ...
        glClearColor(1.f, .0f, .0f, 0.f);
        projection =
            glm::ortho(-1 / 2.f, 1 / 2.f, -1 / 2.f, 1 / 2.f, -far, far);
        glm::mat4 p(0.f);
        p[0][0] = 1.f;
        p[1][2] = 1.f;
        p[2][1] = -1.f;
        ...
        view = glm::mat4(1.f);
        view = glm::translate(view, {0.f, -camera_distance, 0.f});
        view = glm::rotate(view, view_angle, {1.f, 0.f, 0.f});
      }
      break;
      case 1:
      {
        ...
        projection = projection * p;
        view = glm::mat4(1.f);
        view = glm::translate(view, {-camera_distance, 0.f, 0.f});
        view = glm::rotate(view, view_angle, {1.f, 0.f, 0.f});
      }
      break;
      case 2:
