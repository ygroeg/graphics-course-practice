...
{
    float power = -1 + 1 / (roughness * roughness);
...
void main()
{
    ...
    vec3 sun_normalized = normalize(sun_direction);
    vec3 sun_light = (diffuse(sun_normalized) + specular(sun_normalized)) * sun_color;
    vec3 vec_to_light = normalize(point_light_position - position);
    float distance = length(point_light_position - position);
    float attenuation = point_light_attenuation.x +  point_light_attenuation.y * distance +
                        point_light_attenuation.z * distance * distance;
    vec3 point_light = (diffuse(vec_to_light) + specular(vec_to_light))
                          * point_light_color / attenuation;

    vec3 color = ambient + sun_light + point_light;
    out_color = vec4(color, .5f);
}
)";
...
try
{
  ...
  while (running)
  {
    ...
    glm::mat4 projection =
        glm::perspective(glm::pi<float>() / 3.f, 1 / aspect, near, far);
    ...
    glUniform3f(sun_direction_location, 1.0f, 1.0f, 1.0f);
    glUniform3f(sun_color_location, 1.0f, 0.9f, 0.8f);
    glUniform3f(point_light_position_location, cos(time) * width,
                sin(time) * height, 0);
    glUniform3f(point_light_color_location, 1.f, 0.f, 0.f);
    glUniform3f(point_light_attenuation_location, 1, 0.001, 0.0000001);
    glUniform1f(glossiness_location, 0.f);
    ...
    for (int i = 0; i < 9; i++)
    {
      ...
      glUniformMatrix4fv(model_location, 1, GL_FALSE,
                         reinterpret_cast<float *>(&model));
      glUniform1f(glossiness_location, i * 1.5);
      glUniform1f(roughness_location, i * 0.05);
...
try
{
  ...
  GLuint view_location = glGetUniformLocation(program, "view");
  GLuint projection_location = glGetUniformLocation(program, "projection");
  GLuint shadow_projection_location0 =
  ...
  tinyobj::LoadObj(&attrib, &shapes, &materials, nullptr, nullptr,
                   obj_path.c_str(), materials_dir.c_str());
  ...
  auto load_texture = [&](std::string &name)
  {
    ...
    unsigned char *img = stbi_load(texture_path.c_str(), &texture_width,
                                   &texture_height, &texture_channels, 4);
  ...
  while (running)
  {
    ...
    float aspect = (float)height / (float)width;
    glm::mat4 projection = glm::perspective(glm::pi<float>() / 3.f,
                                            (width * 1.f) / height, near, far);
    ...
    glUniformMatrix4fv(projection_location, 1, GL_FALSE,
                       reinterpret_cast<float *>(&projection));
    glUniformMatrix4fv(shadow_projection_location0, 1, GL_FALSE,
                       reinterpret_cast<float *>(&shadow_projection));
    glUniform3fv(camera_position_location, 1, (float *)(&camera_position));
